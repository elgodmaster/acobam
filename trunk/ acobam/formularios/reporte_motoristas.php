...
function Header()
{
    ...
    $this->Ln(5);
	$this->Cell(200,6,'REPORTE DE MOTORISTAS',0,1,'C');
	$this->Ln(5);
    //Salto de línea
...
$cons1 = mysql_query("SELECT  * FROM motorista order by id_motorista",$conexion);
...
$lk=0;
  ...
  $pdf->SetFont('Arial','',10);
	...
	$header=array('Nº','Nombre','Apellido','Telefono','Direccion');
	...
	while($row = mysql_fetch_array($cons1) )
	{
		...
		$pdf->Cell($w[2],6,$row[apellido],'LR',0,'L',$fill);
		$pdf->Cell($w[3],6,$row[telefono],'LR',0,'L',$fill);
		$pdf->Cell($w[4],6,$row[direccion],'LR',0,'L',$fill);
		$pdf->Ln();
		$fill=!$fill;
	}
